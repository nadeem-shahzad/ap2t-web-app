  ...
  try {
    ...
    if (!isFlyerPage(page_key) || typeof image_url !== 'string' || !image_url.trim()) {
      return NextResponse.json(
        { message: 'A valid page and image URL are required' },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (Array.isArray(body.flyers)) {
      ...
      if (
        ...
        flyers.some(
          (flyer) =>
            !Number.isInteger(flyer.id) || !Number.isInteger(flyer.position) || flyer.position < 1
        )
      ) {
      ...
      for (const flyer of flyers) {
        await client.query('UPDATE flyers SET position = position + 1000000 WHERE id = $1', [
          flyer.id,
        ]);
      }
      for (const flyer of flyers) {
        await client.query('UPDATE flyers SET position = $1, updated_at = NOW() WHERE id = $2', [
          flyer.position,
          flyer.id,
        ]);
      }
    ...
    if (!Number.isInteger(id) || typeof image_url !== 'string' || !image_url.trim()) {
      return NextResponse.json(
        { message: 'A valid flyer ID and image URL are required' },
        { status: 400 }
      );
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!Number.isInteger(id))
      return NextResponse.json({ message: 'A valid flyer ID is required' }, { status: 400 });
