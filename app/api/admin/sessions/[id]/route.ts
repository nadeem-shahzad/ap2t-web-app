...
export async function DELETE(
  ...
  try {
    const result = await pool.query(`DELETE FROM sessions WHERE id = $1 RETURNING *`, [session_id]);

    if (result.rows.length === 0) {
      return NextResponse.json({ message: 'Session not found' }, { status: 404 });
    }
    const session = result.rows[0];
    const coachNamesRaw = await pool.query(`SELECT first_name,last_name FROM users WHERE id=$1`, [
      session.coach_id,
    ]);
    ...
    const res = await pool.query(`SELECT user_id from session_players WHERE session_id=$1`, [
      session_id,
    ]);
