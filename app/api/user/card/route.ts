...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const userQuery = await pool.query(
      `SELECT first_name, last_name, email, phone_no FROM users WHERE id = $1`,
      [id]
    );
    ...
    await pool.query(
      `UPDATE users SET square_customer_id = $1, square_card_id  = $2, cardholder_name= $3 WHERE id = $4`,
      [squareCustomerId, squareCardId, cardholder, id]
    );
    ...
    for (const session of sessionsToBePaid) {
      try {
       ...
       WHERE id = $4`,
        ...
        const parentidRaw = await pool.query(`SELECT parent_id FROM players WHERE user_id=$1`, [
          id,
        ]);
        const parent_id = parentidRaw.rows[0];
        const parentMsg = `Payment payed for session ${session.sessionName} paid - $${session.amount}.`;
        sendInAppNotificationBackend(parent_id, parentMsg, `/portal/parent/dashboard`);
      } catch (err) {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    await pool.query(`UPDATE users SET  square_card_id  = $1, cardholder_name= $2 WHERE id = $3`, [
      squareCardId,
      cardholder,
      id,
    ]);
      ...
      try {
       ...
       WHERE id = $4`,
        ...
        if (user) {
          ...
          const parentMsg = `Payment payed for session ${session.sessionName} paid - $${session.amount}.`;
          sendInAppNotificationBackend(parent_id, parentMsg, `/portal/parent/dashboard`);
          sendInAppNotificationBackend(id, parentMsg, `/portal/parent/dashboard`);
        }
      } catch (err) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const userQuery = await pool.query(
      `SELECT first_name, last_name, email, phone_no, square_customer_id, square_card_id FROM users WHERE id = $1`,
      [id]
    );
    const user = userQuery.rows[0] ?? null;
    ...
    if (user?.square_card_id) {
      ...
      finalData = {
        ...response?.card,
        expMonth: response?.card?.expMonth?.toString(),
        expYear: response?.card?.expYear?.toString(),
        version: response.card?.version?.toString(),
      };
    }
