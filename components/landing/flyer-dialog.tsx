...
export default function FlyerDialog({ page }: { page: FlyerPage }) {
  ...
  const showPrevious = () =>
    setCurrentIndex((index) => (index - 1 + visibleFlyers.length) % visibleFlyers.length);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        ...
        className="max-h-[calc(100dvh-50px)] w-[min(94vw,560px)] max-w-none gap-0 overflow-hidden rounded-[18px] border-white/15 bg-[#090909] p-0"
      >
        <DialogTitle className="sr-only">Promotional flyer</DialogTitle>
        <DialogClose asChild>
          <button
            type="button"
            className="absolute right-2.5 top-2.5 z-30 rounded-full border border-white/15 bg-black/55 p-2 text-white/90 shadow-lg backdrop-blur-md transition hover:scale-105 hover:bg-black/80 hover:text-white"
        ...
        <div
          ...
          style={{ backgroundImage: `url(${currentFlyer.image_url})` }}
        />
        ...
        {hasMultipleFlyers && (
          <>
            <button
              ...
              className="absolute left-2.5 top-1/2 z-30 -translate-y-1/2 rounded-full border border-white/15 bg-black/55 p-2.5 text-white/90 shadow-lg backdrop-blur-md transition hover:scale-105 hover:bg-black/80 hover:text-white"
            ...
            <button
              type="button"
              onClick={showNext}
              className="absolute right-2.5 top-1/2 z-30 -translate-y-1/2 rounded-full border border-white/15 bg-black/55 p-2.5 text-white/90 shadow-lg backdrop-blur-md transition hover:scale-105 hover:bg-black/80 hover:text-white"
