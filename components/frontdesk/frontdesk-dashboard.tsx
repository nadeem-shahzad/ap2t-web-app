...
export default function FrontdeskDashboard() {
  ...
  const FRONT_DESK_SESSION_COLUMNS: ColumnDef<FrontDeskActionData>[] = [
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#D1D5DC]">{row.getValue('session_name')}</span>
      ),
    },

    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#D1D5DC]">{row.getValue('player_name')}</span>
      ),
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="text-[#D1D5DC]">
            {bookingDate ? moment(bookingDate).format('YYYY-MM-DD') : 'One-time'}
          </span>
        );
      },
    },
...

    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#D1D5DC]">{row.getValue('referal_code')}</span>
      ),
    },
    {
      accessorKey: 'price',
...

    {
      ...
      cell: ({ row }) => {
        ...
        if (action === 'approval' && status === 'waiting') {
          return <span className="font-medium text-[#D1D5DC]">Requires Approval</span>;
        } else if (action === 'approval' && status !== 'waiting') {
          return <span className="font-medium text-green-500">Done</span>;
        } else if (action === 'cash' && status === 'waiting') {
          return <span className="font-medium text-[#D1D5DC]">Requires Cash Confirmation</span>;
        } else if (action === 'cash' && status !== 'waiting') {
          return <span className="font-medium text-green-500">Done</span>;
        }
      },
          ...
          return (
            <p
              className={`text-sm flex gap-2 items-center ${
                isAccepted ? 'text-primary' : 'text-danger-text'
              }`}
            >
              {isAccepted ? (
  ...
  const filteredData = sessions.filter((item: any) => {
    ...
    const searchWords = debouncedSearch?.toLowerCase().trim().split(/\s+/).filter(Boolean);

    const matchesSearch =
      !searchWords?.length || searchWords.every((word: string) => text.includes(word));
