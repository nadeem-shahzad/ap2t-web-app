...
export default function ReserveComponent({
  ...
  const selectedSessions = useMemo(() => {
    return sessions.filter((s) => {
      ...
      return selected.isSame(start, 'day') || selected.isBetween(start, end, 'day', '[]');
    });
  }, [sessions, selectedDate]);

  const handlePrev = () => {
    setStartDate((prev) => moment(prev).subtract(1, 'day').format('YYYY-MM-DD'));
  };
  ...
  return loading ? (
    ...
    <div className="space-y-6">
      <div className="flex gap-2 justify-center">
        <Button disabled={loading} size="icon" variant="outline" onClick={handlePrev}>
          <ChevronLeft />
        </Button>
              ...
              return (
                <button
                  ...
                  className={`
                          ...
                          transform
                          ${isSelected ? 'bg-white text-black shadow-lg' : undefined}
                        `}
                >
                  <span className="text-xl font-semibold leading-none">
                    {moment(date).format('DD')}
                  </span>
                  <span className="text-xs opacity-70">{moment(date).format('ddd')}</span>
                </button>
              );
            })}
          </div>
        ...
        <Button disabled={loading} size="icon" variant="outline" onClick={handleNext}>
          <ChevronRight />
        </Button>
      </div>
      ...
      <div className="space-y-2 transition-all duration-300">
        {selectedSessions.length === 0 ? (
          <div className="text-sm text-muted-foreground text-center">No sessions found</div>
        ) : (
    ...
    <div className="flex border-b p-4 transition-all gap-2 flex-wrap">
      ...
      <div className="flex flex-col justify-end gap-2">
        ...
        {isEnrolledForSelectedDate ? (
          <Badge className="bg-green-500/10 text-green-400">Enrolled</Badge>
        ) : (
          <ParticipateButton
            player_id={playerId}
