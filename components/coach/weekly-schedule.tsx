...
export const WeeklySchedule: React.FC<WeeklyScheduleProps> = ({ events, id, preference }) => {
  ...
  const getEventForCell = (date: moment.Moment, time: string) => {
    const cellTime = moment(date).set({
      hour:
        parseInt(time.split(':')[0]) +
        (time.includes('PM') && parseInt(time.split(':')[0]) !== 12 ? 12 : 0),
    ...
    const event = events.find((e) => {
      ...
      const startTime = moment(date).set({
        hour:
          ...
          (e.time.includes('PM') && parseInt(e.time.split(':')[0]) !== 12 ? 12 : 0),
      ...
      const endTime = e.end_time
        ? moment(date).set({
            hour:
              ...
              (e.end_time.includes('PM') && parseInt(e.end_time.split(':')[0]) !== 12 ? 12 : 0),
    ...
    <div className="flex flex-1 flex-col p-4 gap-4">
      ...
      <div className="bg-[#1A1A1A] border border-border rounded-[10px] px-4 py-2 flex gap-4">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-active-bg border border-active-text/32 rounded-[3px]"></div>{' '}
          Available
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-danger-bg border border-danger-text/32 rounded-[3px]"></div>{' '}
          Blocked
        </div>
      </div>
            ...
            <ScrollArea className="overflow-x-auto flex flex-1">
              <Table className="relative w-[800px] table-fixed sm:w-full">
                  ...
                  <TableRow className="bg-[#1A1A1A] text-white">
                    ...
                    {weekDates.map((date) => (
                      <TableHead
                        ...
                        className="p-2 border border-[#3A3A3A] font-normal"
                      >
                ...
                <TableBody className="bg-white dark:bg-[#252525]">
                  {timeSlots.map((time) => (
                    <TableRow key={time}>
                      <TableCell className="p-2 border py-4 border-[#3A3A3A] font-normal">
                        {time}
                      </TableCell>
                      {weekDates.map((date) => {
